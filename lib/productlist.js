...
export async function createProduct(data) {
  ...
  return await client.mutate({
    mutation: gql`
    mutation createProduct {
      createProduct(data: {title: "${data.title}", email: "${data.email}", sold: ${data.sold} ownerToken: "${data.ownerToken}", location: "${data.location}", category: {connect: {id: "${data.category}"}}, description: "${data.description}", price: ${data.price}}) {
        id
        title
        sold
      }
